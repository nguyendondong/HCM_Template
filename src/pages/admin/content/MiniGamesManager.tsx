...
interface GameData {
  ...
  completedImage?: string; // for jigsaw puzzle
  gridSize?: { rows: number; cols: number }; // for jigsaw puzzle
}
...
const MiniGamesManager: React.FC<MiniGamesManagerProps> = () => {
  ...
  const fetchGames = async () => {
    try {
      setLoading(true);
      const gamesQuery = query(collection(db, 'mini-games'), orderBy('createdAt', 'desc'));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingGame) {
        await updateDoc(doc(db, 'mini-games', editingGame.id), gameData);
        toast.success('Cập nhật trò chơi thành công!');
      } else {
        await addDoc(collection(db, 'mini-games'), {
          ...gameData,
          createdAt: new Date()
        });
  ...
  const handleDelete = async (gameId: string) => {
    if (window.confirm('Bạn có chắc chắn muốn xóa mini game này?')) {
      try {
        await deleteDoc(doc(db, 'mini-games', gameId));
        await fetchGames();
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredGames.map((game) => {
          const TypeIcon = getTypeIcon(game.type || 'memory');
