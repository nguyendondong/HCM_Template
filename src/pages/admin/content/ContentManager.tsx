...
const ContentManager: React.FC = () => {
  ...
  const contentTypes = [
    ...
    { key: 'mini-games', label: 'Trò chơi Mini', icon: FileText, collection: 'mini-games' },
  ];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error('Có lỗi khi lưu nội dung!');
    }
    // ...existing code...
    try {
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  {activeTab === 'mini-games'
                    ? 'Quản lý các trò chơi mini và cấu hình game'
                    : `Quản lý ${contentTypes.find(t => t.key === activeTab)?.label} của website`
                  }
                </p>
              </div>

              {/* Only show search and create button for non-mini-games tabs */}
              {activeTab !== 'mini-games' && (
