...
export const documentsService = {
  ...
  async getCategory(category: string): Promise<DocumentCategory | null> {
    try {
      const docRef = doc(db, CATEGORIES_COLLECTION, category);
      ...
      console.error(`Error getting category with ID ${category}:`, error);
      throw error;
    }
  },
  ...
  async updateCategory(category: string, categoryData: Partial<DocumentCategory>): Promise<void> {
    try {
      const docRef = doc(db, CATEGORIES_COLLECTION, category);
      ...
      console.error(`Error updating category with ID ${category}:`, error);
      throw error;
    }
  },
  ...
  async deleteCategory(category: string): Promise<void> {
    try {
      ...
      const q = query(collection(db, DOCUMENTS_COLLECTION), where('category', '==', category));
      const querySnapshot = await getDocs(q);
      ...
      await deleteDoc(doc(db, CATEGORIES_COLLECTION, category));
    } catch (error) {
      console.error(`Error deleting category with ID ${category}:`, error);
      throw error;
    }
  },
