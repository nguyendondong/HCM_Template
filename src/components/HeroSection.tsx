...
import { ArrowRight, Star, Heart, Book, Users, MapPin, Clock } from 'lucide-react';
...
const HeroSection: React.FC = () => {
  ...
  // Load thống kê từ service và hero content
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        // Nếu có dữ liệu từ heroContent, sử dụng nó; nếu không thì dùng dữ liệu mặc định
        if (heroContent?.stats && heroContent.stats.length > 0) {
          setStats(heroContent.stats.map(stat => ({
            icon: getIconComponent(stat.icon),
            number: stat.number,
            label: stat.label
          })));
        } else {
          // Fallback với dữ liệu từ heritage service
          setStats([
  ...
  const scrollToTarget = () => {
    const targetSection = heroContent?.actionButton?.targetSection || 'introduction';
  ...
  return (
    <section id="overview" className="min-h-screen relative overflow-hidden bg-gradient-to-br from-red-800 via-red-700 to-red-900 pt-16 lg:pt-20">
      ...
      {(!heroContent || heroContent.backgroundElements?.enableFlags !== false) && (
      ...
      {(!heroContent || heroContent.backgroundElements?.enableStars !== false) && (
      ...
      {(!heroContent || heroContent.backgroundElements?.enableDecorations !== false) && (
        <div className="absolute inset-0">
        ...
        <div className="text-center max-w-6xl pb-2">
          <motion.div
          ...
          >
            {heroContent?.actionButton?.text || "Khám phá ngay"}
